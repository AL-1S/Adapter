...
# 初始化进度计数器
total_files = sum([len(files) for r, d, files in os.walk(dataset_dir) if any(file.endswith("expanded.trans") for file in files)])
processed_files = 0
    ...
    for file in files:
        if file.endswith("expanded.trans.txt"):
            ...
            for line in lines:
                ...
                # 使用 WhisperProcessor 预处理音频文件
                audio_input = whisper_processor(audio_file_path, return_tensors="pt")
                
                # 使用 Whisper 的 encoder 处理预处理后的音频
                with torch.no_grad():
                    encoded_audio = whisper_model(**audio_input).last_hidden_state
                ...
                print(f"Finished processing file {processed_files} of {total_files}")
